...
POST JSON: { tipo: "star"|"matriz", sucursal, entrenador, empleados:[...] }
"""
...
def bdr(thick=False):
    s = Side(style="medium" if thick else "thin",
             color="FFFFDE21" if thick else "FF2A2A2A")
...
def build_sheet(wb, grupos, sucursal, entrenador, empleados, titulo):
    ...
    for g in grupos:
        ...
        if n > 1:
            ws.merge_cells(start_row=4,start_column=ci,end_row=4,end_column=ci+n-1)
        ci += n
    sc(ws,4,ci,"",NEG2)
    ...
    for g in grupos:
        for col in g["cols"]:
            ...
            cell.font      = Font(name="Arial",size=7,bold=True,color=BLANC)
    ...
    for i, emp in enumerate(empleados):
        ...
        pFg  = "FF4CAF50" if pct >= 80 else AMAR if pct >= 50 else ROJO
        ...
        for g in grupos:
            for col in g["cols"]:
                ...
                cell.fill      = fill(STAR_ON_BG if ok else bg)
                cell.font      = Font(name="Arial",size=12,bold=True,
                                      color=AMAR if ok else GRIS2)
    ...
    cell = ws.cell(row=TR,column=2)
    cell.value = "PROMEDIO SUCURSAL"
    cell.fill  = fill(NEG2); cell.font = fnt(AMAR,True,9)
    cell.alignment = aln("left"); cell.border = bdr(True)
    ...
    for col in all_cols:
        ...
        cell.fill = fill(NEG2)
        cell.font = Font(name="Arial",size=7,bold=True,color=BLANC)
        cell.alignment = aln("center"); cell.border = bdr(True)
        ci += 1
    avg = round(spct / len(all_cols)) if all_cols else 0
    aFg = "FF4CAF50" if avg >= 80 else AMAR if avg >= 50 else ROJO
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        fname  = (f"StarPerformance_{sucursal.replace(' ','_')}_{hoy}.xlsx"
                  if tipo == "star"
                  else f"Matriz_{sucursal.replace(' ','_')}_{hoy}.xlsx")
